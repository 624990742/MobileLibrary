...
- (nullable UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if ([self.type isEqualToString:@"0"]) {
        ...
        NSArray *originData = [[NSUserDefaults standardUserDefaults] objectForKey:[MLOCBridgingSwiftMacro ML_Search_kHistroySearchData]];
        if (originData.count == 0) return nil;
        
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, [MLOCBridgingSwiftMacro ML_Search_tagTitleHeighT]+10)];
        view.backgroundColor = [UIColor whiteColor];
        
        UILabel *tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, [MLOCBridgingSwiftMacro ML_Search_tagTitleHeighT])];
