...
- (void)setHotKeyArr:(NSArray *)hotKeyArr {
    ...
    //热门搜索tipLabel
    self.hidden = NO;
    UILabel *tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, [MLOCBridgingSwiftMacro ML_Search_tagTitleHeighT])];
    ...
    for (int i = 0 ; i<_hotKeyArr.count ;i++) {
        ...
        if (i == 0) {
            //第一条，不存在lastTagLabel
            tag.frame = CGRectMake(kTagScreen_margin, [MLOCBridgingSwiftMacro ML_Search_tagTitleHeighT]+kTagVertical_margin, tagSize.width, tagSize.height);
        } else {
