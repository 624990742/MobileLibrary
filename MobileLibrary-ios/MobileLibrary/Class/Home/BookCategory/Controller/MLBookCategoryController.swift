//
//  MLBookCategoryController.swift
//  MobileLibrary
//
...
class MLBookCategoryController: MLBaseController {
    var segmentedDataSource: JXSegmentedTitleDataSource!
    var segmentedView: JXSegmentedView!
    var contentScrollView: UIScrollView!
    var listVCArray = [MLBookListController]()
    ...
    @objc func reloadData() {
        ...
        for index in 0..<segmentedDataSource.titles.count {
            let vc = MLBookListController.init(style: .plain)
    ...
    override func viewDidLayoutSubviews() {
        ...
        segmentedView.frame = CGRect(x: 0, y: ML_NavigationHeight, width: view.bounds.size.width, height: titleHeight)
        contentScrollView.frame = CGRect(x: 0, y: ML_NavigationHeight + titleHeight, width: view.bounds.size.width, height: view.bounds.size.height - ML_NavigationHeight - titleHeight)
    ...
    func getRandomTitles() -> [String] {
        let titles = ["小说", "数理化", "传记", "经济管理", "文学", "历史", "心理学"]
        //随机title数量，4~n
...
extension MLBookCategoryController: JXSegmentedViewDelegate {
